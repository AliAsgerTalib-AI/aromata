...
Do not output raw conversational text or markdown wrappers. Return purely valid JSON adhering strictly to the response schema.`,
    ...
    try {
      ...
      throw new Error(`Invalid JSON response from analysis model: ${e instanceof Error ? e.message : String(e)}`);
    }

    if (!parsedJson.brand || !parsedJson.name) {
      throw new Error('Response missing required fields (brand, name). Schema validation failed.');
    }
