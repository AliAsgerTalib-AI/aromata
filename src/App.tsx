...
const getSeasonalWardrobeRecommendation = (frag: FragranceData): WardrobeRecommendation => {
  ...
  const dominantAccord = frag.accords && frag.accords.length > 0 
    ? frag.accords.reduce((prev, current) => (prev.intensity > current.intensity) ? prev : current).name.toLowerCase()
    : 'fresh';
...
`;
  ...
  const handleSaveCompoundedFragrance = () => {
    if (!compoundingName.trim()) {
      setShelfNotification("Compounding desk: Please designate a name for this custom molecular isolate formula.");
      setTimeout(() => setShelfNotification(null), 4000);
      return;
    }
    ...
    const customSpecimen: FragranceData = {
      ...
      naturalToSyntheticRatio: {
        natural: compoundingIsolates.some(i => i.name.toLowerCase().includes('oakmoss')) ? 10 : 0,
        synthetic: compoundingIsolates.some(i => i.name.toLowerCase().includes('oakmoss')) ? 90 : 100
      },
    ...
    setShelfNotification(`Lab Compound "${customSpecimen.brand} ${customSpecimen.name}" registered to cabinet & activated in Dossier Analyst!`);
  ...
  const handleSaveCurrentToCabinet = () => {
    const exists = cabinet.some(f => f.brand.toLowerCase() === selectedFragrance.brand.toLowerCase() && f.name.toLowerCase() === selectedFragrance.name.toLowerCase());
    if (exists) {
      setShelfNotification(`"${selectedFragrance.brand} ${selectedFragrance.name}" is already catalogued on your spec cabinet!`);
      setTimeout(() => setShelfNotification(null), 4000);
      return;
    }
    const newCabinet = [selectedFragrance, ...cabinet];
    updateCabinet(newCabinet);
    setShelfNotification(`Specimen "${selectedFragrance.brand} ${selectedFragrance.name}" catalogued to Shelf.`);
    setTimeout(() => setShelfNotification(null), 4000);
  };
  ...
  const handleDeleteFromCabinet = (brand: string, name: string) => {
    if (cabinet.length <= 1) {
      setShelfNotification("Dossier shelf preservation: Your cabinet requires at least 1 specimen.");
      setTimeout(() => setShelfNotification(null), 4000);
      return;
    }
    ...
    setShelfNotification(`Removed "${brand} ${name}" from Cabinet Shelf.`);
    setTimeout(() => setShelfNotification(null), 4000);
  };
  ...
  const handleToggleCompare = (brand: string, name: string) => {
    ...
    if (comparedSpecimens.includes(compID)) {
      ...
      if (comparedSpecimens.length >= 2) {
        setShelfNotification("Aromachemical Comparison Limit: Maximum 2 specimens selected. Please deselect one to add another.");
        setTimeout(() => setShelfNotification(null), 4000);
        return;
      }
  ...
  const handleAnalyze = async (e: FormEvent) => {
    ...
    try {
      ...
      const data = await response.json();
      if (!response.ok) {
        throw new Error(data.error || 'Server returned an error running specimen chemical matrix.');
      }

      setSelectedFragrance(data);
      // Immediately include in cabinet shelf automatically if analyzed!
      const existsInCabinet = cabinet.some(f => f.brand.toLowerCase() === data.brand.toLowerCase() && f.name.toLowerCase() === data.name.toLowerCase());
      if (!existsInCabinet) {
  ...
  const handleVerifyBatchCodeDirect = async (e: FormEvent) => {
    ...
    try {
      ...
      const data = await response.json();
      if (!response.ok) {
        throw new Error(data.error || 'Batch code server parser fault.');
      }
                  ...
                  <div className="border-t border-[#2D3139] pt-4 mt-5">
                    <button
                      onClick={() => {
                        ...
                        setActiveTab('dossier');
                        // Push a brief alert
                        setShelfNotification(`Activated "${frag.brand} ${frag.name}" analysis in Dossier tab.`);
                        setTimeout(() => setShelfNotification(null), 4000);
                      }}
                ...
                <div className="bg-[#0D0F14] border border-[#2D3139] p-3.5 rounded-sm flex flex-col md:flex-row items-end gap-3">
                  ...
                  <button 
                    onClick={() => {
                      ...
                      setShelfNotification(`Compounded ${newIsolateParts} parts of "${newIsolateName}" into matrix successfully.`);
                      setTimeout(() => setShelfNotification(null), 3000);
                    }}
